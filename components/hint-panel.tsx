      ...
      {[1, 2, 3].map((level) => {
        ...
        return (
          <div className={`hint-level ${hint ? "open" : ""}`} key={level}>
            ...
            {hint ? <p>{hint.text}</p> : <p>{level === 1 ? "Start with the core behavior before reaching for a fix." : level === 2 ? "A sharper question about the mechanism — still no code." : "Names a location and concept, never a working solution."}</p>}
