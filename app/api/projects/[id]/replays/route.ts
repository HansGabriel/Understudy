...
import { projectIdSchema, projectReplayInputSchema } from "@/lib/schemas";
...
export async function POST(request: Request, context: RouteContext<"/api/projects/[id]/replays">) {
  try {
    ...
    const { sha, guidance } = projectReplayInputSchema.parse(await parseJson(request));
    const result = await createLinkedChallenge(projectId, sha, guidance);
