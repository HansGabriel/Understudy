...
export default function SessionClient({ sessionId }: { sessionId: string }) {
  ...
  const answersTouchedRef = useRef([false, false, false]);
  const explainTouchedRef = useRef(false);
  ...
  const applyLoadedData = useCallback((data: SessionPayload, preserveLocalEdits: boolean) => {
    setPayload(data);
    if (!preserveLocalEdits || !hydratedRef.current) {
      setAnswers(data.session.plan.answers);
      setExplainAnswer(data.session.explainBack.answer);
      hydratedRef.current = true;
    } else {
      setAnswers((current) => current.map((answer, index) => answersTouchedRef.current[index] ? answer : (data.session.plan.answers[index] ?? "")));
      if (!explainTouchedRef.current) setExplainAnswer(data.session.explainBack.answer);
    }
  ...
  }, [applyLoadedData, load]);

  useEffect(() => {
    hydratedRef.current = false;
    answersTouchedRef.current = [false, false, false];
    explainTouchedRef.current = false;
    let cancelled = false;
    void load().then((data) => {
      if (cancelled) return;
  ...
  async function request(path: string, body?: unknown) {
    setError(""); setNotice(""); setNoticeSource(null); setActionNotice(""); setBusy(path);
    try {
  ...
  async function sendCoachMessage() {
    ...
    if (message.length < MIN_INPUT_LENGTH || busy) return;
    setError(""); setNotice(""); setNoticeSource(null); setActionNotice(""); setBusy("/coach");
    try {
  ...
  async function copyPath() {
    ...
    try {
      ...
      setError("");
      setActionNotice("Workspace path copied. Open it in your editor and own the implementation.");
    } catch (reason) {
      setError(`Could not copy the workspace path. ${reason instanceof Error ? reason.message : "Clipboard access is unavailable."}`);
    }
  }
  function openVSCode() {
  ...
  const planNeedsMoreDetail = answers.some((answer) => answer.trim().length < MIN_INPUT_LENGTH);
  const explainNeedsMoreDetail = explainAnswer.trim().length < MIN_INPUT_LENGTH;
    ...
    <AppShell active="session">
      ...
      {!payload ? <section className="page-content"><div className="card plan-card"><p className="eyebrow">Preparing your working copy <span className="technical-label">git worktree</span></p><h2>Opening your replay session...</h2></div></section> : (
        <section className="page-content">
          ...
          {(!planSubmitted || editingPlan) ? <section className="card plan-card"><p className="eyebrow">{editingPlan ? "Plan revision / one round" : "Replay planning checkpoint"}</p><h2>{editingPlan ? "Refine your plan before opening the working copy." : "Rebuild a real change from this project's history."}</h2><p style={{ color: "var(--muted)", marginTop: -8 }}>{editingPlan ? "You get one revision round. The coach will respond once more, then you can lock the plan." : "Name the behavior before opening your working copy. The coach responds with a short question or nudge - never a patch."}</p>{payload.challenge.planQuestions.map((question, index) => <div className="question" key={question}><label htmlFor={`answer-${index}`}>{index + 1}. {question}</label><textarea id={`answer-${index}`} value={answers[index]} aria-describedby={planNeedsMoreDetail ? "plan-validation-hint" : undefined} aria-invalid={planNeedsMoreDetail} onChange={(event) => { answersTouchedRef.current[index] = true; setAnswers((current) => current.map((answer, answerIndex) => answerIndex === index ? event.target.value : answer)); }} onKeyDown={(event) => { if (isSubmitShortcut(event)) { event.preventDefault(); if (!busy && !planNeedsMoreDetail) void submitPlan(); } }} /></div>)}<div className="action-row"><button className="button cobalt" disabled={busy !== "" || planNeedsMoreDetail} onClick={submitPlan}>{busy === "/plan" ? "Coaching..." : editingPlan ? "Submit revision ->" : "Send plan to coach ->"}</button>{editingPlan ? <button className="button secondary" disabled={busy !== ""} onClick={() => setEditingPlan(false)}>Keep current plan</button> : null}</div>{planNeedsMoreDetail ? <p id="plan-validation-hint" className="field-hint" role="status" aria-live="polite">Each answer needs at least {MIN_INPUT_LENGTH} characters.</p> : null}</section> : null}
